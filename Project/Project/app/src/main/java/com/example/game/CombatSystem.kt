...
data class Player(
    ...
    val critBonus: Int = 0,       // เพิ่ม Crit% จากอาวุธ/อุปกรณ์
    val dodgeBonus: Int = 0,      // เพิ่ม Dodge% จากอาวุธ/อุปกรณ์
    val lifestealPct: Int = 0,    // ดูดเลือด % จากอาวุธ (แยกจาก Vampire race)
    val soulRegenBonus: Int = 0,  // SP per turn จากอาวุธ
    val stunChancePct: Int = 0,   // % โอกาส Stun ศัตรูทุกโจมตี (มือ/ค้อน)
    val armorPenPct: Int = 0      // % เจาะเกราะ (ขวาน)
)
...
class CombatManager(
    ...
    private var werewolfHitStreak = 0
    private var specterShieldActive = (player.playerRace == RoguelikeMainActivity.Race.SPECTER)
    private var monkDefStacks = 0
    ...
    private fun executePlayerAttack() {
        ...
        // WEREWOLF: track consecutive hits, every 3rd guarantees a crit
        if (player.playerRace == RoguelikeMainActivity.Race.WEREWOLF) werewolfHitStreak++
        val werewolfForceCrit = player.playerRace == RoguelikeMainActivity.Race.WEREWOLF && werewolfHitStreak % 3 == 0

        // Calculate Crit — include passive crit_chance / crit_bonus from skills + equipment
        ...
        val isCrit = werewolfForceCrit || Random.nextFloat() < (player.luck * 0.05f + skillCritBonus / 100f + player.critBonus / 100f)
        if (player.armorPenPct > 0) addLog("🪖 เจาะเกราะ ${player.armorPenPct}%!", Color.parseColor("#e67e22"))
        var damage = calculateDamage(player.atk, ElementType.PHYSICAL, effectiveDef(enemy.def), playerStatuses, enemy)

        if (isCrit) {
            damage = (damage * 1.5f).roundToInt()
            criticalHits++
            if (werewolfForceCrit) addLog("🐺 เลือดจัด! คริติคอลรับประกัน!", Color.parseColor("#ff6b6b"), true)
            else addLog("⚡ คริติคอล!", Color.parseColor("#f1c40f"), true)
        }
        ...
        // Relic: low_hp_atk_bonus (cracked_skull)
        val relicLowHpAtk = relicInt("low_hp_atk_bonus")
        if (relicLowHpAtk > 0 && player.hp.toFloat() / player.maxHp < 0.5f) {
            damage += relicLowHpAtk
            addLog("💀 กะโหลกหักสาม! HP ต่ำ ATK +$relicLowHpAtk", Color.parseColor("#9b59b6"))
        }

        // TITAN: +1 damage per 10 max HP
        if (player.playerRace == RoguelikeMainActivity.Race.TITAN) {
            val titanBonus = player.maxHp / 10
            damage += titanBonus
            addLog("🦣 ร่างยักษ์! ดาเมจเพิ่ม +$titanBonus", Color.parseColor("#e67e22"))
        }

        // CYBORG: double ATK damage when HP below 30%
        if (player.playerRace == RoguelikeMainActivity.Race.CYBORG && player.hp.toFloat() / player.maxHp < 0.3f) {
            damage *= 2
            addLog("🤖 โอเวอร์โหลด! HP วิกฤต — ดาเมจ x2!", Color.parseColor("#00bcd4"), true)
        }

        // TEMPLAR: +50% damage vs boss enemies
        if (player.classData == RoguelikeMainActivity.CharacterClass.TEMPLAR && enemy.isBoss) {
            damage = (damage * 1.5f).roundToInt()
            addLog("⚜️ แสงศักดิ์สิทธิ์! บอสได้รับดาเมจ x1.5!", Color.parseColor("#FFD700"), true)
        }

        enemy.hp = max(0, enemy.hp - damage)
        damageDealt += damage

        // NINJA: 8% instant kill on non-boss enemies
        if (player.classData == RoguelikeMainActivity.CharacterClass.NINJA && enemy.hp > 0 && !enemy.isBoss && Random.nextInt(100) < 8) {
            enemy.hp = 0
            addLog("🥷 เงาสังหาร! สังหารทันที!", Color.parseColor("#9b59b6"), true)
        }

        // PIRATE: steal gold = 15% of damage dealt
        if (player.classData == RoguelikeMainActivity.CharacterClass.PIRATE && damage > 0) {
            val goldStolen = max(1, damage * 15 / 100)
            activity.gold += goldStolen
            addLog("🏴‍☠️ ปล้น! +$goldStolen 🪙", Color.parseColor("#f1c40f"))
        }

        // Passive lifesteal (skill: lifesteal_passive)
        ...
        // Vampire race: blood drain on normal attack
        applyVampireHeal(damage)

        // Equipment weapon lifesteal
        if (player.lifestealPct > 0 && damage > 0) {
            val weaponHeal = max(1, damage * player.lifestealPct / 100)
            player.hp = min(player.maxHp, player.hp + weaponHeal)
            updatePlayerHp()
            addLog("⚔️ ดูดเลือดจากอาวุธ! +$weaponHeal HP", Color.parseColor("#27ae60"))
        }

        addLog("คุณโจมตี ${enemy.name} สร้างความเสียหาย $damage หน่วย", Color.WHITE)

        // Equipment stun chance
        if (player.stunChancePct > 0 && enemy.hp > 0 && Random.nextInt(100) < player.stunChancePct) {
            getSelectedEnemyStatuses().add(ActiveStatus(StatusEffect.STUN, 1, 0))
            addLog("💫 ${enemy.name} ถูก Stun! (จากอาวุธ)", Color.parseColor("#f1c40f"), true)
            updateEnemyStatus()
        }

        // WITCH: 25% chance to hex enemy (DEFENSE_DOWN for 2 turns)
        if (player.playerRace == RoguelikeMainActivity.Race.WITCH && enemy.hp > 0 && Random.nextInt(100) < 25) {
            getSelectedEnemyStatuses().add(ActiveStatus(StatusEffect.DEFENSE_DOWN, 2, 5))
            addLog("🧙 คำสาปมนต์! ลด DEF ${enemy.name} 5 หน่วย!", Color.parseColor("#9b59b6"))
            updateEnemyStatus()
        }

        // Relic: burn_on_attack (eternal_flame)
        val burnChance = relicInt("burn_on_attack")
        if (burnChance > 0 && enemy.hp > 0 && Random.nextInt(100) < burnChance) {
            getSelectedEnemyStatuses().add(ActiveStatus(StatusEffect.BURN, 2, max(2, player.atk / 5)))
            addLog("🔥 เปลวเพลิงนิรันดร์! ${enemy.name} ติด BURN!", Color.parseColor("#e74c3c"))
            updateEnemyStatus()
        }
        // Relic: enemy_execute_chance (death_ring)
        val execChance = relicInt("enemy_execute_chance")
        if (execChance > 0 && enemy.hp > 0 && enemy.hp.toFloat() / enemy.maxHp < 0.15f && Random.nextInt(100) < execChance) {
            enemy.hp = 0
            addLog("💍 แหวนจ้าวมรณะ! ประหาร ${enemy.name} ทันที!", Color.parseColor("#9C27B0"), true)
        }
    ...
    // ── Vampire race: heal on any damage dealt ──────────────────────────────
    private fun applyVampireHeal(damageDealtToEnemy: Int) {
        if (player.playerRace != RoguelikeMainActivity.Race.VAMPIRE) return
        if (damageDealtToEnemy <= 0) return
        val vampHeal = max(2, damageDealtToEnemy * 12 / 100)
        player.hp = min(player.maxHp, player.hp + vampHeal)
        updatePlayerHp()
        addLog("🧛 ดูดเลือด! ฟื้นฟู +$vampHeal HP", Color.parseColor("#c0392b"))
    }

    // ── Shadow race + skill + equipment dodge check ──────────────────────────
    private fun checkPlayerDodge(attackerName: String): Boolean {
        var dodgeChance = 0
        if (player.playerRace == RoguelikeMainActivity.Race.SHADOW) dodgeChance += 20
        dodgeChance += playerStatuses.filter { it.effect == StatusEffect.DODGE }.sumOf { it.power }
        dodgeChance += player.dodgeBonus
        if (dodgeChance <= 0) return false
        return if (Random.nextInt(100) < dodgeChance) {
            addLog("💨 ${player.name} หลบหลีก! หลบการโจมตีของ $attackerName ได้!", Color.parseColor("#00d4ff"), true)
            true
        } else false
    }
    // ────────────────────────────────────────────────────────────────────────
    ...
    private fun basicEnemyAttack(enemy: Enemy, enemyIndex: Int, bonusAtk: Int = 0) {
        // Check dodge before applying any damage
        if (checkPlayerDodge(enemy.name)) {
            Handler(Looper.getMainLooper()).postDelayed({
        ...
        // SPECTER: full immunity to the first attack of each combat
        if (specterShieldActive) {
            specterShieldActive = false
            addLog("👻 ร่างเงา! ภูมิคุ้มกันการโจมตีครั้งแรก!", Color.parseColor("#9b59b6"), true)
            Handler(Looper.getMainLooper()).postDelayed({
                if (player.hp <= 0 && !combatEnded) defeat()
                else { enemyTurnIndex++; executeEnemyTurn() }
            }, 1000)
        ...
        if (blocked) {
            ...
            // DRAGONKIN: reduce physical damage taken by 5
            val finalDamage = if (player.playerRace == RoguelikeMainActivity.Race.DRAGONKIN) max(0, damage - 5) else damage
            if (player.playerRace == RoguelikeMainActivity.Race.DRAGONKIN && finalDamage < damage) {
                addLog("🐲 เกล็ดมังกร! ลดความเสียหาย ${damage - finalDamage} หน่วย!", Color.parseColor("#e67e22"))
            }
            player.hp = max(0, player.hp - finalDamage)
            damageTaken += finalDamage
            addLog("${enemy.name} โจมตี สร้างความเสียหาย $finalDamage หน่วย", Color.parseColor("#FF3333"))
            val reflectPct = player.unlockedSkills.sumOf { id ->
                val s = SkillTreeData.getAnySkillById(player.classData, player.playerRace, id) ?: return@sumOf 0
                if (s.isPassive) (s.effects["reflect"] as? Int) ?: 0 else 0
            }
            if (reflectPct > 0) {
                val reflectDmg = max(1, finalDamage * reflectPct / 100)
            ...
            // GOLEM: reflect 15% physical damage back to attacker
            if (player.playerRace == RoguelikeMainActivity.Race.GOLEM && finalDamage > 0) {
                val golemReflect = max(1, finalDamage * 15 / 100)
                enemy.hp = max(0, enemy.hp - golemReflect)
                addLog("🗿 หินแกร่ง! สะท้อน $golemReflect ดาเมจกลับ!", Color.parseColor("#95a5a6"))
                updateEnemyHp(enemyIndex)
            }
            // MONK: gain +1 DEF stack when hit (max 10 stacks)
            if (player.classData == RoguelikeMainActivity.CharacterClass.MONK && finalDamage > 0 && monkDefStacks < 10) {
                monkDefStacks++
                player.def++
                addLog("👊 Iron Body! DEF ↑ ($monkDefStacks/10)", Color.parseColor("#3498db"))
            }
        }
    ...
    private fun executeEnemySkill(enemy: Enemy, action: CombatAction, enemyIndex: Int) {
        ...
        // Check dodge before applying any damage
        if (checkPlayerDodge(enemy.name)) {
            Handler(Looper.getMainLooper()).postDelayed({ enemyTurnIndex++; executeEnemyTurn() }, 1000)
            return
        }
        // SPECTER: full immunity to the first attack of each combat
        if (specterShieldActive) {
        ...
        val isLowHp = player.hp.toFloat() / player.maxHp < 0.5f
        val afterIronWill2 = if (isLowHp && lowHpDefBoost > 0) max(1, (rawDmg * (1f - lowHpDefBoost / 100f)).roundToInt()) else rawDmg
        val dmgReduction2 = relicInt("damage_reduction")
        val reducedDmg = if (dmgReduction2 > 0) max(1, (afterIronWill2 * (1f - dmgReduction2 / 100f)).roundToInt()) else afterIronWill2
        ...
        if (blocked) {
            ...
            if (dmgReduction2 > 0 && damage > 0) addLog("🛡️ Relic ลดความเสียหาย $dmgReduction2%!", Color.parseColor("#3498db"))
            if (damage > 0) {
                // DRAGONKIN: reduce physical damage taken by 5
                val finalSkillDmg = if (player.playerRace == RoguelikeMainActivity.Race.DRAGONKIN) max(0, damage - 5) else damage
                if (player.playerRace == RoguelikeMainActivity.Race.DRAGONKIN && finalSkillDmg < damage) {
                    addLog("🐲 เกล็ดมังกร! ลดความเสียหาย ${damage - finalSkillDmg} หน่วย!", Color.parseColor("#e67e22"))
                }
                player.hp = max(0, player.hp - finalSkillDmg)
                damageTaken += finalSkillDmg
                addLog("ได้รับความเสียหาย $finalSkillDmg หน่วย!", Color.parseColor("#FF3333"))
                // Life steal: enemy recovers % of damage dealt
                if (action.lifeSteal) {
                    val steal = max(1, (finalSkillDmg * 0.4f).roundToInt())
                ...
                if (reflectPct > 0) {
                    val reflectDmg = max(1, finalSkillDmg * reflectPct / 100)
                ...
                // GOLEM: reflect 15% physical damage back to attacker
                if (player.playerRace == RoguelikeMainActivity.Race.GOLEM && finalSkillDmg > 0) {
                    val golemReflect = max(1, finalSkillDmg * 15 / 100)
                    val enemyIdx = enemies.indexOf(enemy)
                    if (enemyIdx != -1) {
                        enemy.hp = max(0, enemy.hp - golemReflect)
                        addLog("🗿 หินแกร่ง! สะท้อน $golemReflect ดาเมจกลับ!", Color.parseColor("#95a5a6"))
                ...
                // MONK: gain +1 DEF stack when hit by skill (max 10 stacks)
                if (player.classData == RoguelikeMainActivity.CharacterClass.MONK && finalSkillDmg > 0 && monkDefStacks < 10) {
                    monkDefStacks++
                    player.def++
                    addLog("👊 Iron Body! DEF ↑ ($monkDefStacks/10)", Color.parseColor("#3498db"))
                }
    ...
    private fun processStatusEffects() {
        ...
        while (playerItr.hasNext()) {
            ...
            when (status.effect) {
                StatusEffect.POISON, StatusEffect.BURN, StatusEffect.BLEED -> {
                    val dmgRed = relicInt("damage_reduction")
                    val rawStatusDmg = status.power
                    val statusDmg = if (dmgRed > 0) max(1, (rawStatusDmg * (1f - dmgRed / 100f)).roundToInt()) else rawStatusDmg
                    player.hp = max(0, player.hp - statusDmg)
                    addLog("ความเสียหายจาก ${status.effect.displayName}: $statusDmg", Color.parseColor("#e74c3c"))
                    if (applyRelicDeathProtection()) { /* saved */ } else applyMinHpRelic()
                    updatePlayerHp()
                }
                StatusEffect.REGEN -> {
        ...
        // Passive soul_regen (Mage Soul Well etc.) + equipment soul regen
        var soulRegen = player.soulRegenBonus
        player.unlockedSkills.forEach { id ->
            val skill = SkillTreeData.getAnySkillById(player.classData, player.playerRace, id) ?: return@forEach
        ...
        if (soulRegen > 0) {
            ...
            tvPlayerSoul.text = "SP: ${player.soulPoints}"
            if (player.soulRegenBonus > 0) addLog("🔮 อาวุธฟื้นฟู SP +${player.soulRegenBonus}", Color.parseColor("#9b59b6"))
        }

        // MERMAID: +3 SP per turn automatically
        if (player.playerRace == RoguelikeMainActivity.Race.MERMAID) {
            player.soulPoints += 3
            tvPlayerSoul.text = "SP: ${player.soulPoints}"
            addLog("🧜 กระแสน้ำ! ฟื้นฟู SP +3", Color.parseColor("#3498db"))
        }

        // MONK: +3 HP regen per turn (Iron Body passive)
        if (player.classData == RoguelikeMainActivity.CharacterClass.MONK) {
            val monkRegen = min(3, player.maxHp - player.hp)
            if (monkRegen > 0) {
                player.hp += monkRegen
                addLog("👊 Iron Body! ฟื้นฟู HP +$monkRegen", Color.parseColor("#2ecc71"))
            }
        }
    ...
    private fun victory() {
        ...
        val demonSoulBonus = if (player.playerRace == RoguelikeMainActivity.Race.DEMON) enemies.size * 5 else 0
        val totalSoul = enemies.sumOf { it.soulReward } + enemies.size * soulPerKill + demonSoulBonus
        if (demonSoulBonus > 0) addLog("😈 สัญญาปีศาจ! +$demonSoulBonus SP จากการสังหาร!", Color.parseColor("#9b59b6"))
    ...
    private fun defeat() {
        ...
        // UNDEAD: 15% chance to revive with 1 HP
        if (player.playerRace == RoguelikeMainActivity.Race.UNDEAD && Random.nextInt(100) < 15) {
            player.hp = 1
            updatePlayerHp()
            addLog("💀 คืนชีพ! อันเดดลุกขึ้นอีกครั้งด้วย 1 HP!", Color.parseColor("#9b59b6"), true)
            Handler(Looper.getMainLooper()).postDelayed({ showActionButtons() }, 1200)
            return
        }

        // PHOENIX: revive with 30% HP once per floor
        if (player.playerRace == RoguelikeMainActivity.Race.PHOENIX && !activity.phoenixRevivedThisFloor) {
            activity.phoenixRevivedThisFloor = true
            val reviveHp = max(1, player.maxHp * 30 / 100)
            player.hp = reviveHp
            updatePlayerHp()
            addLog("🔥 เกิดใหม่! ฟีนิกซ์ฟื้นคืนชีพด้วย $reviveHp HP!", Color.parseColor("#e74c3c"), true)
            Handler(Looper.getMainLooper()).postDelayed({ showActionButtons() }, 1200)
            return
        }
    ...
    private fun executeActiveSkill(skill: Skill) {
        val rawSoulCost = skill.cost * 10
        val freeSkillThreshold = relicInt("low_hp_free_skills")
        val isFreeSkill = freeSkillThreshold > 0 && player.hp.toFloat() / player.maxHp * 100 < freeSkillThreshold
        val soulCost = if (isFreeSkill) 0 else rawSoulCost
        ...
        if (isFreeSkill) addLog("🎴 ไพ่มรณะ! HP ต่ำ — สกิลฟรี!", Color.parseColor("#9C27B0"), true)
        ...
        val sorcererMult = if (player.classData == RoguelikeMainActivity.CharacterClass.SORCERER) 1.3f else 1.0f

        // Single-hit or AOE damage — skip this block if multi_hit handles damage instead
        if ((effects.containsKey("aoe_damage") || effects.containsKey("damage")) && !effects.containsKey("multi_hit")) {
            ...
            if (effects.containsKey("def_scaling_damage")) {
                ...
                val damage = calculateDamage(extraDmg, ElementType.PHYSICAL, effectiveDef(enemy.def), enemyStatuses, enemy)
                enemy.hp = max(0, enemy.hp - damage)
                applyVampireHeal(damage)
                if (player.lifestealPct > 0 && damage > 0) {
                    val wHeal = max(1, damage * player.lifestealPct / 100)
                    player.hp = min(player.maxHp, player.hp + wHeal); updatePlayerHp()
                    addLog("⚔️ ดูดเลือดจากอาวุธ! +$wHeal HP", Color.parseColor("#27ae60"))
                }
                addLog("⚖️ ตัดสินด้วยพลังป้องกัน! สร้างความเสียหาย $damage", Color.parseColor("#3498db"))
                handled = true
            } else if (effects.containsKey("aoe_damage")) {
                // Hit all living enemies (armor pen applied per target)
                var totalAoeDmg = 0
                enemies.forEachIndexed { index, e ->
                    if (e.hp > 0) {
                        val baseDmg = (player.atk * damageMultiplier).roundToInt()
                        val damage = calculateDamage(baseDmg, ElementType.PHYSICAL, effectiveDef(e.def), playerStatuses, e)
                        e.hp = max(0, e.hp - damage)
                        totalAoeDmg += damage
                        updateEnemyHp(index)
                    }
                }
                applyVampireHeal(totalAoeDmg)
                if (player.lifestealPct > 0 && totalAoeDmg > 0) {
                    val wHeal = max(1, totalAoeDmg * player.lifestealPct / 100)
                    player.hp = min(player.maxHp, player.hp + wHeal); updatePlayerHp()
                    addLog("⚔️ ดูดเลือดจากอาวุธ! +$wHeal HP", Color.parseColor("#27ae60"))
                }
                addLog("💥 ใช้ท่าโจมตีหมู่!", Color.parseColor("#e74c3c"))
            } else {
                // Hit selected enemy — use playerStatuses so ATK_UP buffs apply
                val baseDmg = (player.atk * damageMultiplier).roundToInt()
                val damage = calculateDamage(baseDmg, ElementType.PHYSICAL, effectiveDef(enemy.def), playerStatuses, enemy)
                enemy.hp = max(0, enemy.hp - damage)

                // Gimmick: Life Drain (Necromancer skill)
                ...
                applyVampireHeal(damage)
                // Equipment weapon lifesteal on skills
                if (player.lifestealPct > 0 && damage > 0) {
                    val wHeal = max(1, damage * player.lifestealPct / 100)
                    player.hp = min(player.maxHp, player.hp + wHeal); updatePlayerHp()
                    addLog("⚔️ ดูดเลือดจากอาวุธ! +$wHeal HP", Color.parseColor("#27ae60"))
        ...
        // Multi-hit (Samurai, Void Strike - Shadow, etc.)
        if (effects.containsKey("multi_hit")) {
            val hits = (effects["multi_hit"] as? Int) ?: 1
            val damageMult = ((effects["damage"] as? Int)?.toFloat()?.div(100f) ?: 1.0f) * sorcererMult
            ...
            repeat(hits) {
                ...
                val damage = calculateDamage(baseDmg, ElementType.PHYSICAL, effectiveDef(target.def), playerStatuses, target)
            ...
            applyVampireHeal(totalDmg)
            if (player.lifestealPct > 0 && totalDmg > 0) {
                val wHeal = max(1, totalDmg * player.lifestealPct / 100)
                player.hp = min(player.maxHp, player.hp + wHeal); updatePlayerHp()
                addLog("⚔️ ดูดเลือดจากอาวุธ! +$wHeal HP", Color.parseColor("#27ae60"))
            }
        ...
        if (effects.containsKey("soul_burst")) {
            ...
            if (spConsumed > 0) {
                val burstDamage = calculateDamage((spConsumed * ratio * sorcererMult).roundToInt(), ElementType.PHYSICAL, enemy.def, playerStatuses, enemy)
        ...
        // Dodge Buff — adds actual dodge chance (%)
        if (effects.containsKey("dodge_buff")) {
            val value = (effects["dodge_buff"] as? Int) ?: 0
            val dur = (effects["duration"] as? Int) ?: 3
            playerStatuses.add(ActiveStatus(StatusEffect.DODGE, dur, value))
            addLog("💨 ร่างหายวับ! โอกาสหลบหลีก $value% เป็นเวลา $dur เทิร์น", Color.parseColor("#00d4ff"))
            updatePlayerStatus()
            handled = true
        }
    ...
    private fun effectiveDef(rawDef: Int): Int =
        if (player.armorPenPct > 0) max(0, (rawDef * (1f - player.armorPenPct / 100f)).toInt())
        else rawDef
